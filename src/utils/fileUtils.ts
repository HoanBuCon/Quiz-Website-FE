...
export const showDuplicateModal = (
  ...
  return new Promise((resolve) => {
    ...
    modal.innerHTML = `
      <div class="bg-white ${isDarkMode ? 'dark:bg-gray-800' : ''} rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
        <div class="flex items-center mb-4">
          ...
          <h3 class="text-lg font-semibold text-gray-900 ${isDarkMode ? 'dark:text-white' : ''}">
            Tên file bị trùng
          </h3>
        </div>
        ...
        <div class="space-y-3 mb-6">
          <label class="flex items-center cursor-pointer p-3 rounded-lg border ${isDarkMode ? 'border-gray-600 hover:bg-gray-700' : 'border-gray-200 hover:bg-gray-50'} transition-colors">
            ...
            <div>
              <div class="font-medium text-gray-900 ${isDarkMode ? 'dark:text-white' : ''}">
                Ghi đè lên File cũ
              </div>
              <div class="text-sm text-gray-500 ${isDarkMode ? 'dark:text-gray-400' : ''}">
          ...
          <label class="flex items-center cursor-pointer p-3 rounded-lg border ${isDarkMode ? 'border-gray-600 hover:bg-gray-700' : 'border-gray-200 hover:bg-gray-50'} transition-colors">
            ...
            <div>
              ...
              <div class="text-sm text-blue-600 ${isDarkMode ? 'dark:text-blue-400' : ''} font-mono">
                → "${suggestedName}"
              </div>
            </div>
          </label>
        ...
        <div class="flex space-x-3">
          ...
          <button id="cancelBtn" class="flex-1 bg-gray-300 ${isDarkMode ? 'dark:bg-gray-600' : ''} text-gray-700 ${isDarkMode ? 'dark:text-gray-200' : ''} px-4 py-2 rounded-lg hover:bg-gray-400 ${isDarkMode ? 'dark:hover:bg-gray-500' : ''} transition-colors font-medium">
            Hủy
          </button>
        </div>
      </div>
    ...
    const radioButtons = modal.querySelectorAll('input[name="duplicateAction"]') as NodeListOf<HTMLInputElement>;
    ...
    confirmBtn.addEventListener('click', () => {
      const selectedAction = Array.from(radioButtons).find(radio => radio.checked)?.value as 'overwrite' | 'rename';
