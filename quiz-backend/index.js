...
async function getPrisma() {
  if (!prisma) {
    ...
    console.log('[INIT] PrismaClient initialized lazily');
  }
  return prisma;
}
...
// Only use lock locally, log-only under Passenger
let acquiredLock = false;
if (!runningUnderPassenger) {
  if (!acquireLock()) {
    console.error('Cannot acquire lock. Exiting...');
...
// ====== Base path setup (compatible with Passenger) ======
const BASE_PATH =
  process.env.PASSENGER_BASE_URI ||
  process.env.BASE_PATH ||
  (isProd ? '/api' : '');
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`Port ${port} is already in use`);
    console.error(
      `Try: killall node (or taskkill /F /IM node.exe on Windows)`
    );
    if (acquiredLock) releaseLock();
    process.exit(1);
  } else {
...
// Dev-only: Nodemon restart cleanup
if (process.env.NODE_ENV === 'development') {
  process.on('SIGUSR2', () => {
    console.log('Nodemon restart detected...');
    releaseLock();
